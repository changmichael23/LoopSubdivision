...
#ifdef _MSC_VER
#pragma comment(lib, "libglew_sharedd.lib")
#endif
...
void SpecialInput(int key, int x, int y)
{
	...
	case GLUT_KEY_RIGHT:
		*currentGraph = Loop::LoopSubdivision(*currentGraph);
		...
		glewInit();
		g_BasicShader.LoadVertexShader("../src/basic.vs");
		g_BasicShader.LoadFragmentShader("../src/basic.fs");
...
bool Initialize()
{
	...
	centerPoints3D.push_back(Point(-250, 0, 250));
	centerPoints3D.push_back(Point(0, 0, -250));
	centerPoints3D.push_back(Point(0, 500, 0));
